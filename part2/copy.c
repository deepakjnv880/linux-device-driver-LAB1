...
{
    struct scull_qset *qs = dev->data;        /* Allocate the first qset explicitly if need be. */
    if (! qs) {
...
ssize_t scull_read(struct file *filp, char __user *buf, size_t count,loff_t *f_pos)
{
    ...
    s_pos = rest / quantum; q_pos = rest % quantum;    /* follow the list up to the right position (defined elsewhere) */
...
ssize_t scull_write(struct file *filp, const char __user *buf, size_t count,loff_t *f_pos)
{
    ...
    if (mutex_lock_interruptible(&dev->mutex))
        return -ERESTARTSYS;    /* Find the list item, qset index, and offset in the quantum. */
    item = (long)*f_pos / itemsize;
    rest = (long)*f_pos % itemsize;
    s_pos = rest / quantum;
    q_pos = rest % quantum;    /* Follow the list up to the right position. */
    dptr = scull_follow(dev, item);
    if (dptr == NULL)
        goto out;
    if (!dptr->data) {
...
int scull_open(struct inode *inode, struct file *filp)
{
    ...
    filp->private_data = dev; /* for other methods */    /* If the device was opened write-only, trim it to a length of 0. */
